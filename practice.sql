...
SELECT 
    ...
    co.region AS region,
    co.surface_area AS surface_area
FROM 
    countries co
WHERE 
    co.continent = 'Europe';
...
SELECT 
    ...
    co.region AS region,
    co.surface_area AS surface_area,
    co.indep_year AS independence_year,
    co.population AS population
FROM 
    countries co
WHERE 
    co.population >= 100000;
...
SELECT 
    ...
    co.indep_year AS independence_year,
    co.population AS population,
    co.life_expectancy AS life_expectancy
FROM 
    countries co
WHERE 
    co.life_expectancy BETWEEN 56 AND 76;
...
FROM 
    cities ci
WHERE 
    ci.country_code IN ('NLD', 'ALB', 'DZA');
...
SELECT MIN(surface_area) AS 'アジアの最小面積'
FROM countries
...
SELECT ce.name AS name, c.name AS name
FROM celebrities ce
...
SELECT 
    ce.name,
    (SELECT c.name FROM countries c WHERE c.code = ce.country_code) AS 国名
FROM celebrities ce;
...
SELECT '1980' AS 誕生年, COUNT(*) AS 人数
FROM celebrities
...
WHERE ce.age IS NOT NULL  -- NULLの年齢は除外
GROUP BY c.name
ORDER BY 平均年齢 DESC;